...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($error)) {
        ...
        $stmt = $conn->prepare("INSERT INTO vehicles (model_name, url, category, motor_type, controller, shocker_size, transmission, max_speed, tyre_size, brake_type, wheel_base, load_capacity, no_of_passengers, mileage, chassis_weight, vehicle_weight, charger_input_voltage, charger_output_voltage, battery_set, single_battery, charging_time, battery_quantity, rear_rest_frame_slide, middle_rest_frame_slide, fog_light, decorative_pipe_light, water_guard_front, water_guard_back, coin_mobile_tray, dala_size, foot_mat, music_system, stepney, other_features, length_mm, width_mm, height_mm, gap_middle_rear_seat, tools, screw_driver, jack_handle, wheel_wrench, spanner, hydraulic_jack) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssssssssssssssssssssssssssssssssssssssss", $model_name, $url, $category, $motor_type, $controller, $shocker_size, $transmission, $max_speed, $tyre_size, $brake_type, $wheel_base, $load_capacity, $no_of_passengers, $mileage, $chassis_weight, $vehicle_weight, $charger_input_voltage, $charger_output_voltage, $battery_set, $single_battery, $charging_time, $battery_quantity, $rear_rest_frame_slide, $middle_rest_frame_slide, $fog_light, $decorative_pipe_light, $water_guard_front, $water_guard_back, $coin_mobile_tray, $dala_size, $foot_mat, $music_system, $stepney, $other_features, $length_mm, $width_mm, $height_mm, $gap_middle_rear_seat, $tools, $screw_driver, $jack_handle, $wheel_wrench, $spanner, $hydraulic_jack);

        if ($stmt->execute()) {
            $vehicle_id = $stmt->insert_id; // Get the newly inserted vehicle ID
            ...
            if (!empty($uploaded_images)) {
                ...
                foreach ($uploaded_images as $image_name) {
                    $stmt_images->bind_param("is", $vehicle_id, $image_name);
                    $stmt_images->execute();
                }
                $stmt_images->close();
            }
...
<main id="main" class="main">
    <?php if ($success) : ?>
        <div class="alert alert-success alert-dismissible fade show d-flex align-items-center gap-2" role="alert" 
            ...
            <span style="color: #166534; font-weight: 500;">Vehicle Added Successfully!</span>
    ...
    <div class="pagetitle mb-4">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h1 style="font-size: 1.75rem; font-weight: 600; color: #374151; letter-spacing: -0.025rem;">Add Vehicle</h1>
                <nav>
                    <ol class="breadcrumb mb-0" style="background: none; padding: 0;">
                        ...
                        <li class="breadcrumb-item active" style="color: #9ca3af; font-weight: 500;">Add Vehicle</li>
                    </ol>
                </nav>
            </div>
            <a href="viewvehicles.php" class="btn" 
                        ...
                        <form method="post" class="mt-2" enctype="multipart/form-data" id="vehicleForm">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-4">
                                    <label for="images" class="form-label" 
                                           style="color: #374151; font-weight: 500;">Images <span class="red" style="color: #ef4444;">*</span></label>
                                    <input type="file" name="images[]" id="images" class="form-control" 
                                           style="border-radius: 8px; border: 1px solid #d1d5db; padding: 0.6rem 1rem; 
                                                  transition: all 0.3s ease;" multiple accept="image/jpeg,image/png" required>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-12">
                                    ...
                                    <textarea name="other_features" class="form-control" 
                                              style="border-radius: 8px; border: 1px solid #d1d5db; padding: 0.6rem 1rem; 
                                                     transition: all 0.3s ease;" rows="3"></textarea>
                                    <small class="form-text text-muted" style="font-size: 0.85rem; color: #6b7280;">
                            ...
                            <div class="row mt-4">
                                <div class="col-sm-12 text-end">
                                    <button type="submit" name="savevehicle" class="btn" 
